...
export interface AgentBus {
  ...
  readonly complete: (nodeId: string, result: AgentResultRecord) => Effect.Effect<void> // fulfils the latch
  ...
  // gather without blocking: park until a watched agent finishes, mail
  // arrives in the waiter's OWN inbox, or the timeout — whichever first
  readonly awaitChange: (opts: {
    readonly waiterKey: string
    readonly watch: ReadonlyArray<string>
    readonly timeoutMs: number
  }) => Effect.Effect<void> // [!code highlight]

  readonly interruptAll: () => Effect.Effect<void> // Esc kills the whole tree
  // … plus supervision: childrenOf/runningChildrenOf, per-agent interrupt, snapshot
}
```

Three communication shapes, each a different question. **`post`/`drain`** is a direct message — an inbox per agent, drained at a turn boundary (more on *why* a turn boundary in a moment). **`boardPost`/`boardRead`** is a shared blackboard: a sub-agent that discovers "the retry config actually lives in `sdk-core`, not here" posts it once and every sibling can read it, so parallel investigations don't each rediscover the same fact. And **`complete`** fulfils a latch — the bus's internal `Deferred`, the one-shot cell a waiter is parked on — then drops a completion note in the parent's inbox and on the board, so even a parent that wasn't waiting hears about it.

`awaitChange` is the centerpiece, and the highlight is why. `wait_for_agents` doesn't block the process; it *races* three things — any watched agent finishing, a message arriving in the waiter's own inbox, or a timeout — and wakes on the first (the tool then reads each watched agent's status, the drained inbox, and the blackboard tail, and hands all three back to the model). So a parent waiting on its writers is still reachable: another agent can message it, a child can report progress, and it wakes for that too, not just for completion. Blocking, but cooperatively — the difference between a thread parked on a mutex and a fiber selecting on three channels.

Two upgrades landed on that gather since this was drafted, both in the direction of honesty. Every running agent now carries live **health** on the bus — `{ state, lastActivityAt, detail }`, where state is `starting` / `generating` / `tool-running` / `retrying` / `awaiting-approval` / `waiting-on-agents` — stamped at the loop's observable edges and published as edge-triggered `agent_health` events (state changes plus coarse activity re-stamps, never a heartbeat). The rows `wait_for_agents` hands back now include that state and the idle seconds, so a parent reads `retrying: 429 2/3 — next in 8s` where it used to stare at a mute timeout. And terminal truth now comes from the persisted tree, not the bus: the bus used to keep a bounded map of finished runs, and the bound was a bug — a child that aged out of it read back as phantom-running, spinning its parent's gathers into the step cap. That map is deleted; the bus carries only live runs, every exit funnels through one terminal path, and a finished agent reports an honest status — `ok`, `partial`, `error`, or `killed` — plus a typed stop reason saying why.
...
The lock was wrong twice over. It couldn't see the overlap that actually bites — an agent writing `packages/sdk-core` and another writing `packages/sdk-core/auth` aren't in the same folder, so a per-folder lock waves them both through; catching that means locking ancestor/descendant ranges, which means holding several locks at once, which means a deadlock risk bought to defend a case the design already discourages. And the failure that finally killed it was worse than a missed overlap: a live fleet wedged when three *read-only* reviewers scoped to the same folder all counted as writers, took the lock in turn, and the first one hung mid-provider-call — holding the lock inside the runtime, where nothing could observe the wait and nothing could recover it. Zero LLM calls, completely stuck, no way out but teardown. So the lock came out, and its job moved to where sequencing is observable and recoverable — the orchestrator (spawn a writer, `wait_for_agents` with its timeout, spawn the next; a stuck writer is a visible row in the tree, not a silently held permit) — with the prompt teaching the discipline:

> **Read in parallel, write one at a time.** Read-only work (research, investigation, review) never conflicts — fan it out, several at once. WRITING work does conflict: there is NO lock, so two sub-agents writing at the same time race and corrupt each other's edits. So when delegating changes, run writers ONE AT A TIME — spawn a coder, `wait_for_agents` until it finishes, then spawn the next; order by dependency.

This is a real philosophical choice, and it's worth saying plainly: **correctness here rests on a prompted discipline, not an enforced invariant.** A model can ignore it and fire two overlapping writers. What makes that acceptable is the shape of the failure and the backstops around it — the work is sandboxed to a folder so the blast radius is bounded, every run is a persisted node you can inspect and roll back, the [shared token budget](/posts/persistent-context-tree/) caps how much a runaway fan-out can spend, and (since this was drafted) every fleet deliverable passes a mandatory independent verify gate before the run returns, so work that raced itself surfaces as a needs-work verdict instead of shipping. The pattern the prompt actually teaches is the one that's both safe and how a careful human would do it anyway: *investigate in parallel first, then implement sequentially.*
...
**Non-blocking is harder to hold in your head.** A blocking `await` has one obvious failure mode: it's slow. An async fleet has subtler ones — a parent that forgets to `wait_for_agents` and finishes its turn while children are still running (the inbox catches their completions at the next boundary, but the *user* may see "done" before "done"), or two agents waiting on each other. The bus's timeout on `awaitChange` is the floor under the worst of it — backed, since this was drafted, by a stall watchdog that force-stops a hung child so it can't strand the tree. The watchdog earned a rewrite of its own: it now reads the health model above instead of a blind progress clock, and kills only a zero-activity model call (`starting`/`generating` past the deadline) — an agent mid-long bash run, mid-retry backoff, or legitimately parked in a 300-second wait is exempt, each of those states bounded by its own timeout. The blind-clock version killed exactly those: a parent lawfully waiting on its writers died as "stalled". And a stalled child is finalized honestly — `killed` if it produced nothing, `partial` with its text preserved if it did. But "what is everyone blocked on?" still becomes a real question, which is why there's a `:tree` view to answer it.
