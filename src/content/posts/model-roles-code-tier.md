...
## The tier is also the failover ladder

A role earns its keep once more when a model dies *mid-run*. Since this was drafted, the router classifies provider failures, and for the persistent kind — quota exhausted for hours, a request shape this model rejects — it retries the call once on a fallback. The fallback rules are written in tier terms: a `code` call falls back to the run's pinned `general` model; a `general` call falls back to `Settings.fallbackModel`, a model the human configured for exactly this moment (unset ⇒ no failover — the error surfaces). Never a model the agent chose, never the selection that just failed. That's the roles argument applied to disaster: because every call already carries its tier, "the coding model is down" has a principled answer — step up one rung — instead of a hardcoded emergency id. And it happens loudly: a notice in the UI, `llm.failover.*` span attributes, and a `[failover: … → … after quota]` note carried into the run's outcome, so a run that limped home on the fallback never reads like a run that didn't.
