...
The loop waits for the run's fresh sub-agent nodes to settle, then `gateOnce` unions their `filesChanged` and asks the Verifier's deliverable gate for a verdict. `sound` accepts. `needs_work` under the attempt cap (default 2) retries with the verifier's concrete reasons injected as the next turn, so the swarm fixes named problems instead of re-rolling the dice — and learning stays *out* of the gate: an earlier cut distilled on every failed attempt, which mined the same conversation twice; the turn-boundary distill is the one distillation a run gets. A broken gate emits a loud `unavailable` event and proceeds: never a silent pass, never an infinite loop. And every round — accept, retry, *and* unavailable, with the verifier's error text — is persisted to a `gate_verdicts` audit table, because the forensics that shaped this design were full of a class you can't debug from memory: `claude` exited 1 after two seconds, three times, and the run silently shipped. Even the scheduled path stopped being gate-free: an unattended cron run gets a one-shot verdict — no retry loop with nobody watching, but never silence either. There is no tool to call and no tool to forget. The prompt can't opt out of the referee any more than your code can opt out of the type checker.
...
**The referee is slow and external.** An Opus review that actually reads files and runs the typecheck takes minutes, and the timeout has now been tuned from both ends: a 3-minute cap killed legitimate reviews mid-flight, a 30-minute one left a half-hour spinner between you and your deliverable. It sits at 10 today, with a 15-minute wall clock over the whole gate phase — past that, the run records a budget-exhausted verdict instead of spinning. And it requires the `claude` binary, logged in, on your PATH. For learnings that's fine — fail-closed just means learning pauses. For deliverables the design is fail-soft by intent: your task never blocks on a missing referee, you just don't get the sign-off.

**The counters only just started counting.** Every delta item carries `(✓✗)` helpful/harmful counters, and for weeks nothing incremented them — dead weight the format carried on faith. As of days ago the signal is wired from both directions: re-learning a constraint bumps its ✓, and the gate prompt now lists the loaded constraint ids so a verdict that *cites* a violated rule bumps its ✗ on the way back. What consumes those counters is still unbuilt — dedup is by name only, and the SkillOps-style maintenance pass ([retire when low-utility *and* duplicated](https://arxiv.org/abs/2605.13716)) remains a design. But the faith part is over: history is accumulating now, and the bullet this post opened with will earn or lose its place in the prompt on the record, not the vibes.
