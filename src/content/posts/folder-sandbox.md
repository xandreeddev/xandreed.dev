...
There's a third answer, and it's smaller than either: a directory. The agent reads anywhere, and writes — files *and* shell — inside one folder. No matrix, no rules file, no image to build. One primitive, one check, and a line that the user, the model, and the filesystem all understand the same way.
