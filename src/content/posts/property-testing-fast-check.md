...
And one level above both sits the question neither can touch: "given this prompt, does the agent read the file before editing it?" That's behavior with meaning, judged on real model output — the eval suite's territory, and [a post of its own](/posts/colocated-evals/). The stack has three floors: properties for algebra, examples for semantics, evals for behavior. Confusing the floors is how you end up asserting string-contains on an LLM transcript and calling it a unit test.
